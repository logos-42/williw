        ...
        try:
            with safe_open(file_path, framework="pt") as f:
                for key in f.keys():
                    ...
                    # 获取张量信息，避免使用 torch.__version__
                    try:
                        tensor_shape = list(tensor.shape)
                        tensor_numel = tensor.numel() if hasattr(tensor, 'numel') else np.prod(tensor_shape)
                    except:
                        tensor_shape = list(tensor.shape)
                    ...
                    print(f"加载层: {key}, 形状: {tensor_shape}, 参数数量: {tensor_numel}")
