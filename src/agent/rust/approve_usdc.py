...
print(f"Current USDC allowance for exchange: {current_allowance / 1e6} USDC")

if current_allowance > 0:
    print("Allowance already set. Skipping approval.")
else:
    # Approve a large amount (max uint256)
    max_uint256 = 2**256 - 1
    approve_txn = usdc_contract.functions.approve(exchange_address, max_uint256).build_transaction({
        'from': address,
    ...
    # Check new allowance
    new_allowance = usdc_contract.functions.allowance(address, exchange_address).call()
    print(f"New USDC allowance for exchange: {new_allowance / 1e6} USDC")
