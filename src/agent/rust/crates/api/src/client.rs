...
impl ProviderClient {
    ...
    pub fn from_model_with_anthropic_auth(
        ...
        match providers::detect_provider_kind(&resolved_model) {
            ...
            ProviderKind::OpenAi => {
                // DashScope (qwen-*) and MiniMax also return ProviderKind::OpenAi
                // because they speak the OpenAI wire format, but they need their
                // own config which reads different env vars and points at their
            ...
            ProviderKind::DeepSeek => {
                // DeepSeek uses OpenAI-compatible API with ANTHROPIC_API_KEY and ANTHROPIC_BASE_URL
                let config = OpenAiCompatConfig::deepseek();
                Ok(Self::OpenAi(OpenAiCompatClient::from_env_with_auth(
                    config,
                    "ANTHROPIC_API_KEY",
                )?))
            }
