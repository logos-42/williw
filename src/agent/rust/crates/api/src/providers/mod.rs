...
pub fn detect_provider_kind(model: &str) -> ProviderKind {
    ...
    // If ANTHROPIC_BASE_URL points to DeepSeek, use DeepSeek provider
...
pub fn model_token_limit(model: &str) -> Option<ModelTokenLimit> {
    ...
    match canonical.as_str() {
        ...
        "deepseek-reasoner" => Some(ModelTokenLimit {
            max_output_tokens: 18_192,
            context_window_tokens: 64_000,
        }),
        s if s.starts_with("deepseek") => Some(ModelTokenLimit {
            max_output_tokens: 8_192,
            context_window_tokens: 64_000,
        }),
        s if s.starts_with("MiniMax") || s.starts_with("minimax") => Some(ModelTokenLimit {
...
mod tests {
    ...
    fn plugin_config_max_output_tokens_overrides_model_default() {
        ...
        std::fs::create_dir_all(cwd.join(".alou")).expect("project config dir");
