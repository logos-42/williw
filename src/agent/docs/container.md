# Container-first alou-code workflows
...
- `rust/crates/rusty-claude-cli/src/main.rs` exposes that state through the `alou sandbox` / `cargo run -p rusty-claude-cli -- sandbox` report.
...
```bash
docker build -t alou-code-dev -f Containerfile .
```

### Podman

```bash
podman build -t alou-code-dev -f Containerfile .
```
...
docker run --rm -it \
  -v "$PWD":/workspace \
  -e CARGO_TARGET_DIR=/tmp/alou-target \
  -w /workspace/rust \
  alou-code-dev \
...
If you want to run `alou` against a second checkout while keeping `alou-code` itself mounted read-write:
...
- Running with `CARGO_TARGET_DIR=/tmp/alou-target` avoids leaving container-owned `target/` artifacts in your bind-mounted checkout.
